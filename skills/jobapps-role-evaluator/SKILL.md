...
description: Use when running blocker preflight for a job and extracting the tailoring map an applicant should use.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
Turn a job link or description into blocker preflight, JD-grounded tailoring requirements, a truthful angle, and the next application action. If the applicant supplied the JD, assume they intend to apply unless a hard blocker appears. Do not fit-score, grade, or produce fake precision.
...
2. **Run blocker preflight**
   ...
   - Seniority clearly too high for the applicant's current search: skip unless they explicitly override.
   ...
   - Do not skip for generic "weak fit" if the applicant supplied the JD; extract tailoring targets instead.

3. **Assume apply intent**
   - If no hard blocker appears, keep `decision: apply` and move into material tailoring.
   - Use `decision: pending` only when a concrete prerequisite must be resolved first, usually sponsorship/work-authorization research, location feasibility, or missing JD/application constraints.
...
5. **Map requirements to truthful proof and tailoring requirements**
   ...
   - If a proof candidate came from Downloads, a CV, `knowledge_base.html`, an old cover letter, or portfolio copy, mark it as candidate/unconfirmed unless the applicant explicitly confirms it or a reliable document supports it.
...
6. **Choose the angle**
   ...
   - For backend/full-stack: reliable operational systems, APIs, auth/proxy work, and database-backed workflows.
...
- Applying is the default when the applicant gives a JD and blockers are clear.
...
- In native Hermes TUI sessions, use `jobapps_start_material_prep` for existing JobApps job IDs when the applicant wants multiple opportunities prepared in parallel.
