...
description: Generic public template for drafting networking messages, referral requests, follow-ups, or contact strategy.
version: 1.0.0
author: Hermes Agent
license: MIT
metadata:
...
- Work-authorization state only if it affects whether outreach is worth doing. Do not lead with it unless the user instructs you to.
- Do not browse or reuse old messages, cover letters, CVs, or private seed files as live material sources. Use the current database record of contacts, proof, and outreach decisions.
...
- mentioning legal/logistics details in the first cold message unless the context requires it
...
- stop after two non-reply follow-ups unless the user has a real new reason to reopen
...
Creating a Gmail draft is allowed when useful. Never send. Use the sender identity configured locally; do not assume a personal, school, alumni, or work email address from public defaults.

## Strategy

- For each strong role, look for recruiters, hiring managers, engineers on the team, founders for small startups, researchers for research-heavy teams, and relevant alumni/community connections from local profile data.
...
- If the relationship path, contact identity, email status, or ask is unclear, ask the user a targeted question instead of inventing context.
- Ignore stale imported leads unless the user explicitly brings them back. If a lead is corrected as stale, record that correction and stop surfacing it.
...
- [ ] It does not introduce legal/logistics details too early.
