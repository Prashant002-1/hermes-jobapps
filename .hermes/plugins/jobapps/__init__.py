...
def _inject_jobapps_context(user_message: str = "", is_first_turn: bool = False, **kwargs: Any) -> dict[str, str] | None:
    ...
    try:
        ...
        lines.append("Use jobapps_* tools for durable app state changes. Discovery candidates must be promoted before tailoring. For networking, use jobapps_find_people to cache public contacts and jobapps_create_gmail_draft only for drafts; never send email.")
        return {"context": "\n".join(lines)}
    except Exception as exc:
